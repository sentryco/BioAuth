...
extension AuthController {
    ...
    * - Description: The `context` property provides access to an `LAContext` instance which is used for managing biometric authentication. It is lazy-loaded, meaning that it is only created when it is first accessed. If the `_context` instance variable is already set, it returns that instance; otherwise, it creates a new `LAContext`, assigns it to `_context`, and then returns it. This property can also be set to nil, allowing the context to be reset if necessary.
