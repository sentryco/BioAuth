 ...
 * - Description: Enumerates the types of biometric authentication methods available on a device, such as Touch ID or Face ID, or indicates that no biometric support is present.
...
extension BioAuthType {
    ...
    * - Description: This property provides a convenient way to determine the type of biometric authentication supported by the current device, if any. It checks the accessibility of biometric authentication and then uses the `LAContext` class to determine the type of biometry available, returning a corresponding `BioAuthType` value.
    * - Throws: An error if biometric authentication is not accessible on the device.
    * - Remark: This method should be used instead of accessing the `type` property directly.
    ...
    * - Description: Retrieves the localized title for biometric authentication based on the device's support for biometric features. If the device supports modern biometric authentication (Face ID or Touch ID), the `normal` closure is executed with the corresponding title. For older devices without biometric support, the `old` closure is executed.
    ...
    * - Example: To get the title for biometric authentication, use `let authTitle = BioAuthType.getAuthTitle(normal: { type in return "Biometric type: \(type)" }, old: { _ in return "Biometric not supported" })`
    * - Parameters:
    ...
    * - Description: This method determines the appropriate title and description for the biometric authentication prompt based on the type of biometric feature available on the device. It uses closures to provide the title and description for either modern or older devices, ensuring compatibility and a tailored user experience.
    * - Returns: The title and description of the biometric authentication prompt.
...
extension BioAuthType {
    ...
    * - Description: Represents the title and description used in the biometric authentication prompt, encapsulating the text that will be displayed to the user when biometric authentication is requested.
    ...
    * - Description: This closure is used to create an `AuthTitle` object, which contains the title and description for the biometric authentication prompt. The type of biometric authentication, such as face or touch, is passed as a parameter to customize the prompt accordingly.
    ...
    * - Description: This struct represents the title and description of the biometric authentication prompt, but with the text formatted as attributed strings. This allows for more complex styling of the text, such as different fonts, colors, or other attributes. The title and description are used in the biometric authentication prompt, providing the user with context for why authentication is being requested.
    * - Parameters:
    *   - title: The title of the biometric authentication prompt as an attributed string.
    *   - description: The description of the biometric authentication prompt as an attributed string.
    */
